...
module.exports = function (_EventEmitter) {
    ...
    function PeerConnection(config, constraints) {
        ...
        _this.on("offer", function (offerMsg) {
            if (!_this.config.connectMediaServer) {
                _this.send("offer", offerMsg);
            } else {
                // send offerMsg at iceEnd event.
                _this.logger.info('Do not send mgsOffer at this time, wait iceEnd ...');
            }
        });
        ...
        _this.on('iceEnd', function (offerMsg) {
            if (_this.config.connectMediaServer) {
                _this.send('offer', offerMsg);
            }
        });
        ...
        _this.on("addStream", function (event) {
            ...
            _this.stream.psid = (_this.id || _this.sid) + "_" + _this.stream.id;
    ...
    _createClass(PeerConnection, [{
        ...
        key: "offer",
        value: function offer(constraints, cb) {
            var _this2 = this;
        ...
        value: function processMessage(msg) {
            ...
            if (msg.type === "offer") {
                ...
                this.processMsgAnswer(msg.payload, function (err) {
                    if (err) {
                        _this4.logger.error('Cannot process msgAnswer:', err, msg);
                    }
                });
            } else if (msg.type === "iceCandidate") {
                this.processMsgCandidate(msg.payload, function (err) {
                    if (err) {
                        _this4.logger.error('Cannot process msgCandidate:', err, msg);
                    }
                });
            } else {
...
module.exports = function (_EventEmitter) {
    ...
    _createClass(RoomRTC, [{
        ...
        key: "joinMediaServer",
        value: function joinMediaServer(name) {
            var _this4 = this;
            ...
            this.roomName = name;
            return new Promise(function (resolve, reject) {
                var username = _this4.config.username || _this4.connectionId;
                var pc = _this4.webrtc.createPeerConnection({
                    id: _this4.connectionId,
                    connectMediaServer: _this4.config.connectMediaServer
                });
                _this4.emit("peerCreated", pc);
                pc.start();
        ...
        value: function createRoom(name) {
            ...
            return new Promise(function (resolve, reject) {
                _this5.connection.emit('create', name, function (err, roomName) {
                    if (err) {
                        _this5.emit("error", err);
                    } else {
                        _this5.roomName = roomName;
                        _this5.emit("roomCreated", roomName);
        ...
        value: function initMediaSource(mediaConstraints, devName) {
            ...
            var constrains = mediaConstraints || this.config.media;
            return navigator.mediaDevices.getUserMedia(constrains).then(function (stream) {
                ...
                _this6.webrtc.addLocalStream(stream);
                return stream;
            });
        }
    }, {
